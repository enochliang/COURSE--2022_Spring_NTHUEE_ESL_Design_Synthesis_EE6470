...
void Testbench::do_gaussian() {
  ...
  for (y = 0; y <= height; ++y) {
    ...
    // Receive a row of processed data
    if(y > 0){
      for (x = 0; x != width; ++x) {
        if(i_r_result[x].num_available()==0) wait(i_r_result[x].data_written_event());
        r_total = i_r_result[x].read();
        if(i_g_result[x].num_available()==0) wait(i_g_result[x].data_written_event());
        g_total = i_g_result[x].read();
        if(i_b_result[x].num_available()==0) wait(i_b_result[x].data_written_event());
        b_total = i_b_result[x].read();
        //cout << "Now at " << sc_time_stamp() << endl; //print current sc_time
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + x) + 2) = r_total;
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + x) + 1) = g_total;
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + x) + 0) = b_total;
      }
    }
